...
class OLLGAFactL1L2TheoryEnv(OLLGATheoryEnv):
    """
    ...
    def __init__(self, config, test_env=False) -> None:
        ...
        """
        super(OLLGAFactL1L2TheoryEnv, self).__init__(config)
    ...
    def step(self, actions, **kwargs):
        ...
        if stop is False:
            # flip r bits
            mutation_size, crossover_size = actions
...
class OLLGATheoryL1L2EnvDiscrete(OLLGATheoryEnv):
    """OLLGA environment where the choices of lambda is discretised."""

    def __init__(self, config, test_env=False):
        """Init env."""
        super(OLLGATheoryL1L2EnvDiscrete, self).__init__(config, test_env)
        assert (
    ...
    def step(self, action, **kwargs):
        ...
        return super(OLLGATheoryL1L2EnvDiscrete, self).step(action_value, **kwargs)


class OLLGAFactL1L2TheoryEnvDiscrete(OLLGAFactL1L2TheoryEnv):
    """OLLGA environment where the choices of lambda is discretised."""

    def __init__(self, config, test_env=False):
        """Init env."""
        super(OLLGAFactL1L2TheoryEnvDiscrete, self).__init__(config, test_env)
        assert (
            "action_choices" in config
        ), "Error: action_choices must be specified in benchmark's config"
        assert isinstance(
    ...
    def step(self, actions, **kwargs):
        """Take step."""
        lbd1_idx, lbd2_idx = actions
        action_value = [
            self.action_choices[self.inst_id][0][lbd1_idx],
            self.action_choices[self.inst_id][1][lbd2_idx],
        ]
        return super(OLLGAFactL1L2TheoryEnvDiscrete, self).step(action_value, **kwargs)


## PPO Versions
class OLLGAL1L2TheoryPPOEnvDiscrete(OLLGAFactL1L2TheoryEnv):
    """OLLGA environment where the choices of lambda is discretised."""

    def __init__(self, config, test_env=False):
        """Init env."""
        super(OLLGAL1L2TheoryPPOEnvDiscrete, self).__init__(config, test_env)
        assert (
            "action_choices" in config
        ), "Error: action_choices must be specified in benchmark's config"
        assert isinstance(
    ...
    def step(self, actions, **kwargs):
        ...
        return super(OLLGAL1L2TheoryPPOEnvDiscrete, self).step(action_value, **kwargs)
