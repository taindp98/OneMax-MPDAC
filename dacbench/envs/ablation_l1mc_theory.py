...
class OLLGAFactL1MCTheoryEnv(OLLGATheoryEnv):
    """
    ...
    def __init__(self, config, test_env=False) -> None:
        ...
        """
        super(OLLGAFactL1MCTheoryEnv, self).__init__(config)
    ...
    def step(self, actions, **kwargs):
        ...
        if stop is False:
            # flip r bits
            mutation_size, alpha, beta = actions
...
class OLLGAFactL1MCTheoryEnvDiscrete(OLLGAFactL1MCTheoryEnv):
    """OLLGA environment where the choices of lambda is discretised."""

    def __init__(self, config, test_env=False):
        """Init env."""
        super(OLLGAFactL1MCTheoryEnvDiscrete, self).__init__(config, test_env)
        assert (
            "action_choices" in config
        ), "Error: action_choices must be specified in benchmark's config"
        assert isinstance(
    ...
    def step(self, actions, **kwargs):
        """Take step."""
        lbd1_idx, mutation_idx, crossover_idx = actions
        action_value = [
        ...
        return super(OLLGAFactL1MCTheoryEnvDiscrete, self).step(action_value, **kwargs)


## PPO Versions
class OLLGAL1MCTheoryPPOEnvDiscrete(OLLGAFactL1MCTheoryEnv):
    """OLLGA environment where the choices of lambda is discretised."""

    def __init__(self, config, test_env=False):
        """Init env."""
        super(OLLGAL1MCTheoryPPOEnvDiscrete, self).__init__(config, test_env)
        assert (
            "action_choices" in config
        ), "Error: action_choices must be specified in benchmark's config"
        assert isinstance(
    ...
    def step(self, actions, **kwargs):
        ...
        return super(OLLGAL1MCTheoryPPOEnvDiscrete, self).step(action_value, **kwargs)
