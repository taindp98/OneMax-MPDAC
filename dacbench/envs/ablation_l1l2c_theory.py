...
## PPO Versions
class OLLGAL1L2CTheoryPPOEnvDiscrete(OLLGAFactL1L2CTheoryEnv):
    """OLLGA environment where the choices of lambda is discretised."""

    def __init__(self, config, test_env=False):
        """Init env."""
        super(OLLGAL1L2CTheoryPPOEnvDiscrete, self).__init__(config, test_env)
        assert (
            "action_choices" in config
        ), "Error: action_choices must be specified in benchmark's config"
        assert isinstance(
    ...
    def step(self, actions, **kwargs):
        ...
        return super(OLLGAL1L2CTheoryPPOEnvDiscrete, self).step(action_value, **kwargs)
