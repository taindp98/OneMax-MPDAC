...
class OLLGAFactL1TheoryEnv(OLLGATheoryEnv):
    ...
    def __init__(self, config, test_env=False) -> None:
        ...
        """
        super(OLLGAFactL1TheoryEnv, self).__init__(config)
...
class OLLGATheoryL1EnvDiscrete(OLLGATheoryEnv):
    """OLLGA environment where the choices of lambda is discretised."""

    def __init__(self, config, test_env=False):
        """Init env."""
        super(OLLGATheoryL1EnvDiscrete, self).__init__(config, test_env)
        assert (
    ...
    def step(self, action, **kwargs):
        ...
        return super(OLLGATheoryL1EnvDiscrete, self).step(action_value, **kwargs)


class OLLGAFactL1TheoryEnvDiscrete(OLLGAFactL1TheoryEnv):
    """OLLGA environment where the choices of lambda is discretised."""

    def __init__(self, config, test_env=False):
        """Init env."""
        super(OLLGAFactL1TheoryEnvDiscrete, self).__init__(config, test_env)
        assert (
            "action_choices" in config
        ), "Error: action_choices must be specified in benchmark's config"
        assert isinstance(
