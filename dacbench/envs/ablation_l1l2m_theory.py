...
class OLLGAFactL1L2MTheoryEnv(OLLGATheoryEnv):
    """
    ...
    def __init__(self, config, test_env=False) -> None:
        ...
        """
        super(OLLGAFactL1L2MTheoryEnv, self).__init__(config)
    ...
    def step(self, actions, **kwargs):
        ...
        if stop is False:
            # flip r bits
            mutation_size, alpha, crossover_size = actions
            mutation_rate = np.float64(alpha * mutation_size / self.n)
            crossover_rate = np.float64(1.0 / crossover_size)
            ## clip rate to [0,1]
...
class OLLGATheoryL1L2MEnvDiscrete(OLLGATheoryEnv):
    """OLLGA environment where the choices of lambda is discretised."""

    def __init__(self, config, test_env=False):
        """Init env."""
        super(OLLGATheoryL1L2MEnvDiscrete, self).__init__(config, test_env)
        assert (
    ...
    def step(self, action, **kwargs):
        ...
        return super(OLLGATheoryL1L2MEnvDiscrete, self).step(action_value, **kwargs)


class OLLGAFactL1L2MTheoryEnvDiscrete(OLLGAFactL1L2MTheoryEnv):
    """OLLGA environment where the choices of lambda is discretised."""

    def __init__(self, config, test_env=False):
        """Init env."""
        super(OLLGAFactL1L2MTheoryEnvDiscrete, self).__init__(config, test_env)
        assert (
            "action_choices" in config
        ), "Error: action_choices must be specified in benchmark's config"
        assert isinstance(
    ...
    def step(self, actions, **kwargs):
        """Take step."""
        lbd1_idx, mutation_idx, lbd2_idx = actions
        action_value = [
        ...
        return super(OLLGAFactL1L2MTheoryEnvDiscrete, self).step(action_value, **kwargs)


## PPO Versions
class OLLGAL1L2MTheoryPPOEnvDiscrete(OLLGAFactL1L2MTheoryEnv):
    """OLLGA environment where the choices of lambda is discretised."""

    def __init__(self, config, test_env=False):
        """Init env."""
        super(OLLGAL1L2MTheoryPPOEnvDiscrete, self).__init__(config, test_env)
        assert (
            "action_choices" in config
        ), "Error: action_choices must be specified in benchmark's config"
        assert isinstance(
    ...
    def step(self, actions, **kwargs):
        ...
        return super(OLLGAL1L2MTheoryPPOEnvDiscrete, self).step(action_value, **kwargs)
