...
class OLLGAFactL1CTheoryEnv(OLLGATheoryEnv):
    """
    ...
    def __init__(self, config, test_env=False) -> None:
        ...
        """
        super(OLLGAFactL1CTheoryEnv, self).__init__(config)
    ...
    def step(self, actions, **kwargs):
        ...
        if stop is False:
            # flip r bits
            mutation_size, beta = actions
            crossover_size = mutation_size
            mutation_rate = np.float64(1.0 * mutation_size / self.n)
            crossover_rate = np.float64(1.0 * beta / crossover_size)
            ## clip rate to [0,1]
...
class OLLGAFactL1CTheoryEnvDiscrete(OLLGAFactL1CTheoryEnv):
    """OLLGA environment where the choices of lambda is discretised."""

    def __init__(self, config, test_env=False):
        """Init env."""
        super(OLLGAFactL1CTheoryEnvDiscrete, self).__init__(config, test_env)
        assert (
            "action_choices" in config
        ), "Error: action_choices must be specified in benchmark's config"
        assert isinstance(
